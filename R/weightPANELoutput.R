#' An object outputted by weightPANEL function
#' 
#' Objects of class \code{weightPANELoutput} are created by the \code{weightPANEL} function
#'
#' An object of the class `weightPANELoutput' has the following slots:
#' \itemize{
#' \item \code{df} List of dataframes of subsetted PANEL data with imputed data for non-response in covariates, with column for weights - if method none chosen, of class data.frame
...
#' @rdname weightPANELoutput
#' @aliases weightPANELoutput-class initialize,weightTAPS-method getdf,weightTAPSoutput-method getattrit,weightTAPSoutput-method getstats,weightTAPSoutput-method print,weightTAPSoutput-method show,weightTAPSoutput-method plot,weightTAPSoutput-method weightTAPSoutput
#' @details
...
#' Each quantity represents the percentage of people (by demographic group) that attritted the selected panel through the waves specified.
...
#' and Gender, Age, Region, Ethnicity and Education (for ANES).
...
#' @export
setClass(Class="weightPANELoutput", 
...
setMethod("initialize", "weightPANELoutput", 
          function(.Object, ...){
...
#' @export
setGeneric("getdf",
           function(object="weightPANELoutput")  {
             standardGeneric("getdf")
           }
)

setMethod("getdf", "weightPANELoutput",
          function(object){ 
            return(object@df)
          }
)
...
setMethod("getattrit", "weightPANELoutput",
          function(object){ 
...
setMethod("getstats", "weightPANELoutput",
          function(object){ 
            return(object@stats)
          }
)
...
#' @export
setMethod(f="print", "weightPANELoutput",
...
#' @export
setMethod(f="show", "weightPANELoutput",
...
#' @export
setMethod(f="plot", "weightPANELoutput",
          definition=function(x){
            statWaves<-x@stats
            ...
            dotchart(as.matrix(unlist(x@attrit))[,1],labels=names((unlist(x@attrit))),cex=0.5, 
                       main="Atrrition Rates", col=rainbow(length(unlist(x@attrit))-2, star=0.7, end=0.1))
            if(x@type=="TAPS"){
              namesSocio<-c("Age/Gender", "Ethnicity", "Education", "Region", "Income", "Internet users")
            }
            else{
              namesSocio<-c("Gender", "Age", "Region", "Ethnicity", "Education")
            }
